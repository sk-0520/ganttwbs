...
const faviconSourceFilePath = path.join(
	rootDirectoryPath,
	"resource",
	"ganttwbs.svg",
);
const faviconOutputSvgFilePath = path.join(
	rootDirectoryPath,
	"public",
	"favicon.svg",
);
...
(async () => {
	...
	for (const iconSize of iconSizes) {
		const faviconOutputPngFilePath = path.join(
			rootDirectoryPath,
			"public",
			`favicon-${iconSize}x${iconSize}.png`,
		);
	...
	fs.writeFileSync(
		path.join(rootDirectoryPath, "public", "favicon.ico"),
		iconBuffer,
	);
})();
