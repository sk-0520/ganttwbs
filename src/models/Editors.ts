...
import { type TimelineIdOrObject, Timelines } from "@/models/Timelines";
...
export abstract class Editors {
	...
	public static scrollView(
		timeline: TimelineIdOrObject | undefined,
		...
		const columnElement = date
			? document.getElementById(Timelines.toDaysId(date))
			: null;
		const rowElement = timeline
			? document.getElementById(Timelines.toRowId(timeline))
			: null;
	...
	public static scrollTimeline(timeline: TimelineIdOrObject): void {
		...
		const height =
			...
			mainContentElement.clientHeight -
			crossHeaderElement.clientHeight;
