...
import { isValueUnit, type ValueUnit } from "@/models/data/Design";
...
export abstract class Designs {
	private static readonly PropertyRegex =
	...
	public static convertStyleClasses(
		obj: object,
		parents: ReadonlyArray<string>,
	): Map<ClassName, Map<Property, Value>> {
		const result = new Map<ClassName, Map<Property, Value>>();

		const propertyValues = new Map<Property, Value>();

		for (const [selectorOrProperty, valuesOrNestedBlock] of Object.entries(
			obj,
		)) {
	...
	private static convertStylesheetBlock(
	...
	public static convertStylesheet(
		styleClasses: Map<ClassName, Map<Property, Value>>,
	): string {
		const newLine = "\r\n";
