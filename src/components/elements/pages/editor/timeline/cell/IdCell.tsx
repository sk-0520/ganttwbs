...
import type { AnyTimeline, Progress } from "@/models/data/Setting";
...
const IdCell: FC<Props> = (props: Props) => {
	const className = Timelines.getReadableTimelineIdClassName(
		props.readableTimelineId,
	);
	...
	const canSelect =
		...
		selectingBeginDateAtomReader.data.canSelect(props.currentTimeline);

	return (
		<td
			className={classNames("timeline-cell timeline-id", {
				dragging:
			...
			<label>
				{selectingBeginDateAtomReader.data ? (
					<>
						...
						<IconImage
							kind={
								props.isSelectedPrevious
									? IconKind.CheckBoxTimelinePreviousOn
									: IconKind.CheckBoxTimelinePreviousOff
							}
					...
					<IconImage
						kind={IconKind.TimelineGroup}
						fill={completed ? "#555" : undefined}
					/>
				) : (
					<IconImage
						kind={IconKind.TimelineTask}
						fill={completed ? "#555" : undefined}
					/>
				)}
