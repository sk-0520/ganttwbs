...
const TimelineHeaderRow: FC<Props> = (props: Props) => {
	...
	function handleMouseEnter() {
		if (
			!draggingTimelineAtomReader.data &&
		...
		<tr
			...
			className={classNames(
				...
				"_dynamic_programmable_cell_height",
				{
					...
					"selected-previous": selectingBeginDateAtomReader.data?.previous.has(
						props.currentTimeline.id,
					),
			...
			onDragEnter={(ev) =>
				draggingTimelineAtomReader.data?.onDragEnter(ev, props.currentTimeline)
			}
			onDragOver={(ev) =>
				draggingTimelineAtomReader.data?.onDragOver(ev, props.currentTimeline)
			}
			onDragLeave={(ev) =>
				draggingTimelineAtomReader.data?.onDragLeave(ev, props.currentTimeline)
			}
			onDrop={(ev) =>
				draggingTimelineAtomReader.data?.onDrop(ev, props.currentTimeline)
			}
