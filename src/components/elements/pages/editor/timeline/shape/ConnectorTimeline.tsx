...
const ConnectorTimeline: FC<Props> = (props: Props) => {
	...
	const currentWorkRange = workRangesAtomReader.data.get(
	...
	const currentTimeSpanRange = Charts.getTimeSpanRange(
		calendarInfoAtomReader.data.range.begin,
		currentWorkRange,
	);
	const currentChartArea = Charts.createChartArea(
	...
	const currentColor = Charts.getTaskBackground(
		props.currentTimeline,
		resourceInfoAtomReader.data.memberMap,
	...
	return (
		<>
			{props.currentTimeline.previous.map((b) => {
				...
				const previousTimeline = Require.get(
					totalTimelineMapAtomReader.data,
					b,
				);

				const previewColor = Settings.maybeGroupTimeline(previousTimeline)
					? Charts.getGroupBackground(
							previousTimeline,
							rootTimelineReader.data,
							settingAtomReader.data.theme,
						)
					: Charts.getTaskBackground(
							previousTimeline,
							resourceInfoAtomReader.data.memberMap,
							settingAtomReader.data.theme,
						);

				const previewWorkRange = workRangesAtomReader.data.get(
					previousTimeline.id,
				);
				...
				const previousTimeSpanRange = Charts.getTimeSpanRange(
					calendarInfoAtomReader.data.range.begin,
					previewWorkRange,
				);
				const previousChartArea = Charts.createChartArea(
					previousTimeSpanRange,
					previousIndex,
					cell,
					props.areaSize,
				);
				...
				const fromHeadTail =
					currentChartArea.x === previousChartArea.x + previousChartArea.width;
				...
				const connecterColorId = Charts.toConnecterColorId(
					b,
					props.currentTimeline.id,
				);
				const markerId = {
				...
				return (
					<g key={b}>
						<defs>
							...
							<linearGradient id={connecterColorId}>
								<stop
									className="previous"
									offset="0%"
									stopColor={previewColor}
								/>
								<stop
									className="current"
									offset="100%"
